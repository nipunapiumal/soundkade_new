<?php
	...
	function tryAndLoginWithFacebook( $get ) {
		...
		if ( isset( $get['error'] ) ) { // error comming from facebook GET vars
			...
			//$getAccessToken = $accessTokenInfo['fb_response']['access_token'];

			//print_r($getAccessToken);

			// $sql = mysqli_query($conn, "SELECT * FROM customer_reg WHERE fb_access_token='$getAccessToken' LIMIT 1");
			...
			if ( $accessTokenInfo['has_errors'] ) { // there was an error getting an access token with the code
				...
				if ( !$fbUserInfo['has_errors'] && !empty( $fbUserInfo['fb_response']['id'] ) && !empty( $fbUserInfo['fb_response']['email'] ) ) { // facebook gave us the users id/email
					...
					$userInfoWithId = getRowWithValue( 'customer_reg', 'fb_user_id', $fbUserInfo['fb_response']['id'] );

					// check for user with email
					$userInfoWithEmail = getRowWithValue( 'customer_reg', 'email', $fbUserInfo['fb_response']['email'] );

					if ( $userInfoWithId || ( $userInfoWithEmail && !$userInfoWithEmail['password'] ) ) { // user has logged in with facebook before so we found them
						// update user
						updateRow( 'customer_reg', 'fb_access_token', $_SESSION['fb_access_token'], $userInfoWithId['id'] );
						$userInfo = getRowWithValue( 'customer_reg', 'id', $userInfoWithId['id'] );

						$_SESSION['email'] = $userInfo['email'];
						$_SESSION['id'] = $userInfo['id'];
						$_SESSION['val'] = $userInfo['val'];
						...
						$userInfo = getRowWithValue( 'customer_reg', 'id', $userId );
