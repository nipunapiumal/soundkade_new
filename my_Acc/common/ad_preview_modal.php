<div id="ad_<?php echo $row['pid']; ?>" class="modal fade" tabindex="-1"> 
	...
	<div class="modal-content">
		...
		<table class="table table-bordered">
			...
			$data = unserialize($serialized_data); // Show the unserialized data; ?>
			 ...
			 <?php 
			  ...
			  ?>
              <p id="data_list">  <strong><?php echo $sqlf['name']; ?>:</strong> <?php echo $value; ?></p>
			...
			if(mysqli_num_rows($sqlp) > 0) { ?>	
                ...
                <div class="row">
                    <div class="col-xs-12">
                        <!-- PAGE CONTENT BEGINS -->
                        <div>
                            <ul class="gallery-listed">                              
                                <?php 
							  	while($rowp = mysqli_fetch_array($sqlp)) { 
								$img_path = "../post_ad/uploads/".$row['post_id']."/".$rowp['img']; ?>                               
                                <li class="col-md-4 col-sm-6 grid-item post format-image">
                                    <!--<a href="<?php echo $img_path; ?>" data-rel="colorbox">
                                        <img width="160"  src="<?php echo $img_path; ?>" />
                                    </a>-->
                                    <div class="grid-item-inner"><a href="<?php echo $img_path; ?>" data-rel="prettyPhoto[41]" class="media-box"><img src="<?php echo $img_path; ?>"></a></div>
                                </li>
                               <?php } ?>
                            </ul>
		...
		<button class="btn btn-md btn-danger pull-right" data-dismiss="modal"> <i class="ace-icon fa fa-times"></i> Close </button>
		</div>
	</div>
	<!-- /.modal-content --> 
  </div> 
