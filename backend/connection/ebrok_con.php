...
if (array_key_exists('save_ebro', $_POST)) {//check that the relevent code of controller
    $pass = $_POST['pass'];
    $password =  sha1($pass);
    $sql = "INSERT INTO  `erbo_register` ( 
...
//echo $sql; 
    ...
    if ($data) {
	$sql1 = mysqli_query($conn, "INSERT INTO user SET id=NULL,user='" . $_POST['u_name'] . "',val='3', pass='" . $_POST['pass'] . "',name='eBroker',password='$password'");
...
if (array_key_exists("nic_validation", $_POST)) {
    ...
    $sql = "SELECT
count(erbo_register.nic) as nic_count
FROM
erbo_register  WHERE erbo_register.`nic`='$nic'";
...
/* * **********************************  Update EBRO Location **************************************** */

if (array_key_exists('update_ebro_location', $_POST)) {

    $ebro_id = htmlspecialchars(trim($_POST['ebro_id']), ENT_QUOTES, 'UTF-8');
    $lng = htmlspecialchars(trim($_POST['lng']), ENT_QUOTES, 'UTF-8');
    $lat = htmlspecialchars(trim($_POST['lat']), ENT_QUOTES, 'UTF-8');
	
    $sql = "UPDATE erbo_register SET lat='$lat' , lng='$lng' WHERE erbo_id = '$ebro_id'";
	$data = mysqli_query($conn, $sql); //check the result of query
