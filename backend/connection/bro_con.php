...
if (array_key_exists('save_bro', $_POST)) {//check that the relevent code of controller
//save in database
    $sql = "INSERT INTO  `bro_register` ( 
  `bro_id`,
  `name`,
  `address`,
  `nic`,
...
if (array_key_exists('get_city', $_POST)) {
    //$city_did=$_POST['id'];
    $city_did=htmlspecialchars(trim($_POST['id']), ENT_QUOTES, 'UTF-8');
...
if (array_key_exists("nic_validation", $_POST)) {
    ...
    $sql = "SELECT
count(bro_register.nic) as nic_count
FROM
bro_register  WHERE bro_register.`nic`='$nic'";
