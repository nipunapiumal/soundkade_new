...
WHERE fields_map.form_id='$sub_cat' ORDER BY fields_map.`order` ASC";
					    $resultl = mysqli_query($conn, $sqll);
					    //echo $sqll;
					    while ($rowl = mysqli_fetch_array($resultl)) {
						
						//echo $fid = $row1['field_id'];
						
						?>
					    
    					    <div class="form-group"> 
    						<label><?php echo $rowl['name']; ?></label>
						<?php if (($rowl['type'])=='Select') { ?>
    						<select class="form-control" >
						    
						  
						    
						   <?php //$fid = $row1['field_id'];
						    //$field_id = $row1['field_id'];
						    //$sub_cat = $_POST["preview"];
						    
						   $sql3 = "SELECT * FROM form_data WHERE `field_id`=".$rowl['field_id']." AND `form_id`=".$sub_cat ;
						  
						$sql4 = mysqli_query($conn, $sql3);
						
						while($row4 = mysqli_fetch_array($sql4)) { ?>
						<option><?php echo $row4['data']; ?></option>
