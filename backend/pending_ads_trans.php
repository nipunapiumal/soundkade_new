...
if (!isset($_SESSION)) {
   ...
   // $rbo = $_SESSION['user'];
    $rbo = htmlspecialchars(trim($_SESSION['user']), ENT_QUOTES, 'UTF-8');

    $sqlu = mysqli_fetch_assoc(mysqli_query($conn, "SELECT district FROM rbo_register WHERE u_name='$rbo'"));
    $dist = $sqlu['district'];
...
form_prev_data.id = '$post_id'"));
    								...
    								<tr> 
    								    ...
    								    <td>
    									<select class="form-control" id="consultant" style="width:100%">
	...
	?>
    									...
    									<span id="val_consult" style="display:none" class="formError"><font color="#F44336">Please select Consultant</font></span>    
    								    </td>

    								    <td>
    									<button class="btn btn-success btn-xs transfer_btn" value="<?php echo $trans_id; ?>"> &nbsp;<i class="ace-icon fa fa-check-square-o"></i>&nbsp; </button>
...
<?php } ?>
	...
	<script>
		...
		if (validate_both) {
			...
			$.post("connection/rbo_ads_trans.php", {trans: 'data', trans_id: trans_id, consultant: consultant, hidden_trans_id: hidden_trans_id}, function (delMsg) {//delete item
				...
				if (valueDel.msgType === 1) {
					...
					function (isConfirm) {
					    if (isConfirm) {
						window.location.href = "pending_ads_trans.php";
					    }
					});
				    }, 200);
