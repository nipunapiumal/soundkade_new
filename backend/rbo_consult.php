...
if (!isset($_SESSION)) {
    ...
    $sql_id = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM rbo_register WHERE u_name='$user'"));
    $rbo_id = $sql_id['rbo_id'];
    $dis_id = $sql_id['district'];
				...
				<div class="row">
					...
					<div class="widget-box col-md-12 col-xs-12">
						...
						<div class="widget-main col-md-12 col-xs-12"> 
						    ...
						    <div class="table-header">Attachments</div>
...
consult_upload.flag,
cus_transaction.id AS customer_trans_id,
cus_transaction.consult_id,
consult_register.con_id,
consult_register.district,
consult_register.name,
cus_transaction.add_id,
cus_transaction.trans_id,
transaction_type.id AS transaction_id,
transaction_type.name AS trans_name
FROM
consult_upload
Inner Join cus_transaction ON consult_upload.cus_trans_id = cus_transaction.id
Inner Join consult_register ON consult_register.con_id = cus_transaction.consult_id
Inner Join transaction_type ON transaction_type.id = cus_transaction.trans_id
WHERE consult_upload.flag=0 AND consult_register.district='$dis_id'
...
form_prev_data.id = '$post_id'"));
    								...
    								<tr> 
    								    ...
    								    <td><a class="btn btn-success btn-xs" data-toggle="modal" href="#ad_<?php echo $row['pid']; ?>"> &nbsp;<i class="ace-icon fa fa-check-square-o"></i>&nbsp; </a>
								    ...
								    <td><img src="upload/pdf/<?php echo $row1['upload_name']; ?>" width="70" height="70"></td>
								    <td><button class="btn btn-success btn-xs accept_btn" value="<?php echo $row1['id']; ?>"> &nbsp;<i class="ace-icon fa fa-check-square-o"></i>&nbsp; </button></td>
    								    <td><button class="btn btn-danger btn-xs reject_btn" value="<?php echo $row1['id']; ?>"> &nbsp;<i class="ace-icon fa fa-check-square-o"></i>&nbsp; </button> </td>
	...
	<script>
		...
		$('.accept_btn').click(function () {   //when press Delete button in the table		    
			...
			$.post("connection/rbo_consult_con.php", {accept_ad: 'data', id: id}, function (delMsg) {//delete item
				...
				if (valueDel.msgType === 1) {
					...
					function (isConfirm) {
					    if (isConfirm) {
						window.location.href = "rbo_consult.php";
					    }
					});
				    }, 300);
		...
		$('.reject_btn').click(function () {   //when press Delete button in the table		    
			...
			$.post("connection/rbo_consult_con.php", {reject_ad: 'data', id: id}, function (delMsg) {//delete item
			    $.each(delMsg, function (index, valueDel) {
				if (valueDel.msgType === 1) {
				    //swal("Deleted!", "Item has been deleted successfully ", "success");
				    setTimeout(function () {
