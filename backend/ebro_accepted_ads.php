                                        ...
                                        <div class="widget-box col-md-12 col-xs-12">
                                            <div class="widget-body">
                                                <div class="widget-main col-md-12 col-xs-12"> 
                                                    ...
                                                    <div class="table-header"> Reserve Ads </div>
...
WHERE
form_prev_data.flag = 3 AND
form_prev_data.ebro_accept='$ebro'");
...
price
 WHERE `price`.`cat_id`=" . $row['cat']
									...
									?>
    									...
    									<input type="hidden" id="cat_id" value="<?php echo $row['cat']; ?>">
    									<input type="hidden" id="ebroker_add_fee_<?php echo $row['pid']; ?>" value="<?php echo $add_price; ?>">
    									<button class="btn btn-danger btn-xs finalize_btn" value="<?php echo $row['pid']; ?>"> &nbsp;<i class="ace-icon fa fa-check-square-o"></i>&nbsp; </button>
	    ...
	    /*var amount_validate = true;
	     ...
	     var add_pri = $("#ebroker_add_fee").val();
	     var ebro_balance = parseInt(ebro_bal);
	     var add_price = parseInt(add_pri);
		...
		var ad_id = $(this).val();
		var ebro_bal = $("#ebroker_balance").val();
		var add_pri = $("#ebroker_add_fee_" + ad_id).val();
		var ebro_balance = parseInt(ebro_bal);
		var add_price = parseInt(add_pri);
		//var tot=(ebro_balance-add_price);
		//alert(tot); 
		if (ebro_balance >= add_price) {
			...
			$.post("connection/ebro_ads_con.php", {finalize_ad: 'data', ad_id: ad_id, ebro_id: ebro, cat_id: cat}, function (delMsg) {//delete item
				...
				if (valueDel.msgType === 1) {
					...
					function (isConfirm) {
					    if (isConfirm) {
						window.location.href = "ebro_accepted_ads.php";
					    }
					});
				    }, 300);
		...
		if (ebro_balance < add_price) {
		    swal("Something Went Wrong", "Your Account balance not enough to finalize this Add", "warning");
		}
	    });
