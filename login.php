...
if ( isset( $_GET['state'] ) && FB_APP_STATE == $_GET['state'] ) { // coming from facebook
	...
	if ($fbLogin['status'] == 'ok' ) {
		//print_r($_SESSION['email']);
		loggedInRedirect();
		//header("Location: post_ad/index.php");
	}
                                ...
                                <div class="col-md-4 col-sm-4 registerform">
                                    ...
                                    <form id="login-form" method="post">
										...
										<div style="text-align: right;" class="text-center">
												<a href="<?php echo getFacebookLoginUrl(); ?>" class="fb_btn btn btn-primary" ><i class="fa fa-facebook-square" aria-hidden="true"></i> Login with Facebook</a>
										</div>
										<br>
		...
		function submitForm() {
			...
			success: function (data) {
				...
				if (data.status === 'ok') {
					...
					$('#errorDiv').delay(300000).slideUp(100);
					...
					$('#errorDiv').delay(350000).slideUp(100);
					$('#submit_btn').html('<span class="glyphicon glyphicon-log-in"></span> &nbsp; Register Now');
					$('#submit_btn').removeAttr('disabled');
				    });
				}
