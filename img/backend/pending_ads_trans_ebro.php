...
if (!isset($_SESSION)) {
    ...
    $sqlu = mysqli_fetch_assoc(mysqli_query($conn, "SELECT district FROM erbo_register WHERE u_name='$user'"));
    $dist = $sqlu['district'];
...
WHERE
cus_transaction.trans_flag =  '2' AND
...
form_prev_data.id = '$post_id'"));
    								...
    								<tr> 
    								    ...
    								    <td>
    									<button class="btn btn-success btn-xs accept_btn" value="<?php echo $id; ?>"> &nbsp;<i class="ace-icon fa fa-check-square-o"></i>&nbsp; </button>
	...
	<script>
		...
		swal({
		    ...
		    $.post("connection/ebro_ads_trans_con.php", {accept_ad: 'data', trans_id: trans_id}, function (delMsg) {//delete item
			$.each(delMsg, function (index, valueDel) {
				...
				setTimeout(function () {
				    ...
				    function (isConfirm) {
					if (isConfirm) {
					    window.location.href = "pending_ads_trans_ebro.php";
					}
				    });
				}, 300);
