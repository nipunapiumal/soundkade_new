...
if (empty($log)) {
	...
	$sqlu="SELECT * FROM user WHERE user='$log' and val='2'";
	...
	if($countu==1){
		...
		//if($_POST['submit']){
		 ...
		 // $bcode = $_POST['bcode'];
		  ...
		  //$dev_id = $_POST['dev_id'];
		  $dev_id = htmlspecialchars(trim($_POST['dev_id']), ENT_QUOTES, 'UTF-8');
		  //$reg_mobile = $_POST['reg_mobile'];
		  $reg_mobile = htmlspecialchars(trim($_POST['reg_mobile']), ENT_QUOTES, 'UTF-8');
		 ...
		 // $city = $_POST['city'];
		  ...
		  $redirectUrl = "../search_eBroker.php";
		  echo "<script type=\"text/javascript\">";
		  echo "window.location.href = '$redirectUrl'";
		  echo "</script>";
