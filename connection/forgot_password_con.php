...
if (array_key_exists('email', $_POST)) {
	...
	if(mysqli_num_rows($result)<1){//this means current there isn't any profile associated with this email
		...
		//$reset_url = "http://localhost/soundkade/reset_password.php?reset_id=$reset_hash";
		...
		if(mysqli_query($conn, $insertquery)){//send email only if query is successful
			...
			$subject = "Password reset link for Soundkade.lk account {$_POST['email']}";
			$contents = "
			<p>Dear {$row['name']},</p>
			<p>A password reset request has been received for your soundkade.lk account on ".date("Y-m-d, g:i a", $currenttimestamp).". Please <strong><a href='$reset_url'>click here</a></strong> to go to the password reset page. Please note that this reset link is only valid for $linkvalidtimeinhours hours.
			</p>
