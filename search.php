...
if(isset($_GET['district'])){
    $district = htmlspecialchars(trim($_GET['district']), ENT_QUOTES, 'UTF-8');
}
if(isset($_GET['city'])){
...
//echo $MySQL;
 //UDARA: get total number of records before adding limit query
$Mysqltemp =str_replace("SQL_CALC_FOUND_ROWS", "", $MySQL);
//echo  "<br><br>";

$resulttotal = @mysqli_query($conn, $Mysqltemp);
...
//echo "BBBBBBBBBBBBBBBBB<BR>{$resulttotal->num_rows}";
$total_records = "";
//$pagination->records($rows['rows']);
$pagination->records($resulttotal->num_rows);
                            ...
                            <div class="col-md-12">
                                <div id="search">
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <select name="district" class="form-control" id="district" style="border-radius: 20px">
                                            	<option selected disabled value="">Select District</option>

<?php

 $sqlp = mysqli_query($conn, "SELECT * FROM district ORDER BY district ASC");


						    while ($rowp = mysqli_fetch_array($sqlp)) {
						        if(isset($district)){
                                    if($district == $rowp['did']){

                                        echo "<option selected value='{$rowp['did']}'>{$rowp['district']}</option>";



                                    }else{

                                         echo "<option value='{$rowp['did']}'>{$rowp['district']}</option>";
                            ...
                            ?>
                                        ...
                                        <div class="col-md-3">
                                            ...
                                            <input type="hidden" id="cat" value="<?php echo $id; ?>">
                                            ...
                                            <div class="input-group input-group-lg" style="width:100%">
                                                ...
                                                <select name="city" class="form-control" id="drop_city"  style="border-radius: 20px">
...
$query ="SELECT cid, city from city WHERE did={$district} ORDER BY city ASC";
$result789 = mysqli_query($conn, $query);
    echo "<option value=''>All Cities</option>";
    while ($row = mysqli_fetch_array($result789)) {

        if(isset($city)){
            if($city == $row['cid']){
                echo "<option selected value='{$row['cid']}'>{$row['city']}</option>";
     ...
     echo "<option selected disabled>Search District First</option>";
}
...
?>
                                        ...
                                        <div class="col-md-3">
                                            ...
                                            <div class="input-group input-group-lg" style="width:100%">
                                                ...
                                                <select name="subcat" class="form-control" id="drop_subcat"  style="border-radius: 20px">
													<option disabled selected value="">Select SubCategory</option>

						    <?php
                            if(isset($_GET['cat']) && $_GET['cat']!= ""){
                                 $sql = mysqli_query($conn, "SELECT * FROM sub_category WHERE cat_id={$_GET['cat']}");
                            }
                            else{
                ...
                echo "<option value='{$row['id']}'>{$row['sub_category']}</option>";
                                        ...
                                        <div class="col-md-3">
                                            <div class="input-group input-group-lg">
                                                <input type="text" class="form-control" name="keyword"  value=""  style="border-radius: 20px 0 0 20px"/>

                                                <span class="input-group-btn" >
                                                    <button style="border-radius: 0px 20px 20px 0px" type ="submit" name ="submit" class="btn btn-primary search-btn" id="submit"><i class="fa fa-search fa-lg"></i></button>
                                                </span> </div>
                                        </div>
                                    </form>
                                </div>
                ...
                <div id="content" class="content full">
                    <div class="container">
                        <div class="row">
                            ...
                            <div class="sidebar right-sidebar col-md-3 hidden-xs hidden-sm" id="sidebar-col" style="font-size: 15px;">
						...
						<?php //} ?>
						    ...
						    <?php if ($id != 1) { ?>
    						    <div class="accordion-inner"><a href="search.php?cat=1">Soundkade.lk Speakers</a></div>
						    <?php } ?>
						    <?php if ($id != 4) { ?>
    						    <div class="accordion-inner"><a href="search.php?cat=4">Stock Clearance</a></div>
						    <?php } ?>
						    <?php if ($id != 2) { ?>
    						    <div class="accordion-inner"><a href="search.php?cat=2">Other Speakers</a></div>
						    <?php } ?>
						    <?php if ($id != 3) { ?>
    						    <div class="accordion-inner"><a href="search.php?cat=3">Musical Instruments</a></div>
						    <?php } ?>
						    <?php if ($id != 5) { ?>
    						    <div class="accordion-inner"><a href="search.php?cat=5">Accessories</a></div>
						    <?php } ?>
						    <?php if ($id != 6) { ?>
    						    <div class="accordion-inner"><a href="search.php?cat=6">Other</a></div>
						    <?php } ?>
					...
					while ($rowse = mysqli_fetch_array($result)) {
    					...
    					<li class="type-rent col-md-12 col-xs-12 col-sm-12 col-lg-12" style="margin-bottom:5px">
    						...
    						<a href="more_details.php?id=<?php echo $rowse['id']; ?>&e_bro=<?php echo $rowse['ebro_id']; ?>&user=<?php echo $rowse['user']; ?> " class="property-featured-image">
    						    <img src="<?php echo $img_path; ?>" class="common-img wp-post-image img-thumbnail" alt="This is an iamge of <?php echo $rowse['ad_name']; ?>" sizes="(max-width: 600px) 100vw, 600px" />
    						</a>
							</div>
                           ...
                           <div class="col-xs-12 col-sm-12  col-md-12  col-lg-12" style="margin-top:10px">
    						    <h4 class="product-name" style="font-size:25px;margin-top:20px;color:#222"><a style="color:#222" href="more_details.php?id=<?php echo $rowse['id']; ?>&e_bro=<?php echo $rowse['ebro_id']; ?>&user=<?php echo $rowse['user']; ?>"><?php echo $rowse['ad_name']; ?></a></h4>
    						    <span  style="padding: 0;color: #222;font-size:15px;margin-top:20px"><i class="fa fa-map-marker" aria-hidden="true" style="color: #222; font-size: 15px; margin-right: 5px;"></i><?php echo $rowse['street'] . " " . $sqlc['city']; ?></span><br>
                                <span  style="color: #222; font-family: 'Open Sans', sans-serif!important; font-style: italic;font-size:15px;margin-top:20px"><i class="fa fa-clock-o" aria-hidden="true" style="color: #222; font-size: 15px; margin-right: 5px;"></i><?php echo $rowse['date']; ?></span><br>

								<h3 class="product-price" style="margin-top:50px;text-align:right">
							<?php if ($rowse['cat'] != 10 && $rowse['cat'] != 11) { ?>
							    <div class="" style="color:#eb2e2f"><strong>Rs. <?php
                                //echo $price[0];
                                if ($price[2]=='.') {
...
?>
        ...
        $('#district').change(function() {
            ...
            $.ajax({
                url: "connection/get_cities.php",
                type: "POST",
                data: {district:document.getElementById("district").value},
                success: function(result){
                   //alert(result);
                   //console.log(result);
                  // result.push('<option value='all'>All Cities</option>');
                   document.getElementById('drop_city').innerHTML = result;
                },
                error: function(){
                   //alert('failure');
                }});
